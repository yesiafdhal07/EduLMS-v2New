...
async def run_test():
    ...
    try:
        ...
        # -> Open the login page by clicking the 'Masuk Portal' link (element index 98).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[2]/header/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the email (index 1539) and password (index 1543) with invalid credentials, click the submit button (index 1546), wait for the page to respond, then extract visible page text to look for an authentication error message.
        frame = context.pages[-1]
        # Input text
